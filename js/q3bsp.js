...
q3bsp_base_folder = "games";
// q3bsp_base_folder = "games" + "/" + pakName;
...
q3bsp.prototype.loadShaders = function (sources) {
    ...
    for (var i = 0; i < sources.length; ++i) {
        ...
        console.log('Q3BSP loadShaders', sources[i]);
    }
...
q3bsp.prototype.processEntities = function (entities) {
    console.log('Q3BSP processEntities', entities);
    ...
    if (entities.worldspawn[0].music) {
        ...
        // for some reason songs names are separated by a sapce
        let songs = entities.worldspawn[0].music.split(' ');
        console.log('Songs', songs);
        // songs[0] is music?
        // songs[1] is loop?
        this.bgMusic = new Audio(q3bsp_base_folder + '/' + songs[0]);
        // TODO: When can we change this to simply setting the 'loop' property?
        this.bgMusic.addEventListener('ended', function () {
            this.currentTime = 0;
            // if we have a second sound file, play it as a loop
            if (songs && songs[1]) {
                this.src = (q3bsp_base_folder + '/' + songs[1]);
                this.play();
            } else if (songs && songs[0]) {
                // play the first file without stoping
                this.src = (q3bsp_base_folder + '/' + songs[0]);
                this.play();
            }
        }, false);
        this.bgMusic.play();
...
function q3bspCreateSpeaker(speaker) {
    ...
    speaker.audio = new Audio(q3bsp_base_folder + '/' + speaker.noise);
...
q3bsp.prototype.draw = function (viewMat, projMat) {
    this.drawViews([{ viewMat: viewMat, projMat: projMat }]);
}

q3bsp.prototype.drawViews = function (views) {
    var viewCount = views.length;
    if (viewCount == 1 && views[0].viewport) {
        this.setViewport(views[0].viewport);
    }
    ...
    if (this.surfaces.length > 0) {
            ...
            if (q3glshader.setShader(gl, this.skyShader)) {
                for (var j = 0; j < this.skyShader.stages.length; ++j) {
                    ...
                    for (var v = 0; v < viewCount; ++v) {
                        if (viewCount > 1)
                            this.setViewport(views[v].viewport);
                        this.bindSkyMatrix(shaderProgram, views[v].viewMat, views[v].projMat);
        ...
        if (this.defaultSurfaces.length > 0 || this.unshadedSurfaces.length > 0) {
            ...
            for (var v = 0; v < viewCount; ++v) {
                if (viewCount > 1)
                    this.setViewport(views[v].viewport);
                this.bindShaderMatrix(shaderProgram, views[v].viewMat, views[v].projMat);
